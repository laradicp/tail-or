...
def valid_date(x):
    if "/" not in str(x):
        return datetime.datetime.strptime("2024-W1-1", "%Y-W%W-%w")
...
def preprocess():
    ...
    data.to_csv(f"data/timeseries/data_semanal.csv", index=False)


def final():
    data = pd.read_csv(f"data/timeseries/data_semanal.csv")
    ...
    df.to_csv("data/timeseries/timeseries_semanal.csv")
